...
class DynamicClientStatesController extends AppController {
    ...
    public $name       = 'DynamicClientStates';
    public $components = array('Aa','Formatter','GridFilter');
    public $uses       = array('DynamicClientState','User');
    protected $base    = "Access Providers/Controllers/DynamicClientStates/";
...
//------------------------------------------------------------------------
    ...
    public function index(){
        ...
        if(count($q_r) > 1){
            ...
            foreach($q_r as $item){
                if($counter != 0){
                    $previous_time  = $q_r[($counter-1)]['DynamicClientState']['created'];
            ...
            //Add the last one
            $state_now      = $q_r[($counter-1)]['DynamicClientState']['state'];
