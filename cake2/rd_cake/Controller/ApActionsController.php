...
class ApActionsController extends AppController {
    ...
    public $name       = 'ApActions';
    public $components = array('Aa','Formatter','GridFilter');
    public $uses       = array('ApAction','User');
    protected $base    = "Access Providers/Controllers/ApActions/";
...
//------------------------------------------------------------------------
    ...
    public function add() {
        ...
        if (array_key_exists('ap_id', $this->request->data)) {
		    if($this->request->data['ap_id'] != ''){
			    $this->{$this->modelClass}->create();
	...
	public function restart_aps(){
		...
		foreach($this->request->data['aps'] as $n){
			...
			if($already == 0){ //Nothing waiting - Create a new one
				$d	= array('ap_id' => $ap_id,'command' => 'reboot');
				$this->ApAction->create();
				$this->ApAction->save($d);
				$this->ApAction->id = null;
			}
		}
