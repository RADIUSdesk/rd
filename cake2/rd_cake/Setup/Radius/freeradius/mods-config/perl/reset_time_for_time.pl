...
sub authorize {
    ...
    if($RAD_CHECK{'Rd-Reset-Type-Time'} =~ /never/i){ #Return immediately 
        return RLM_MODULE_NOOP;
    }

    if($RAD_CHECK{'Rd-Reset-Type-Time'} =~ /monthly/i){
        $RAD_CHECK{'Rd-Start-Time'} = start_of_month()
    }
    if($RAD_CHECK{'Rd-Reset-Type-Time'} =~ /weekly/i){
        $RAD_CHECK{'Rd-Start-Time'} = start_of_week()
    }
    if($RAD_CHECK{'Rd-Reset-Type-Time'} =~ /daily/i){
        $RAD_CHECK{'Rd-Start-Time'} = start_of_day()
    }

    #Add a dynamic reset interval where the start time will be now minus the interval
    if($RAD_CHECK{'Rd-Reset-Type-Time'} =~ /dynamic/i){
...
sub start_of_day {
    ...
    if(exists($RAD_CHECK{'Rd-Reset-Hour-Time'})){
        my $reset_hour = $RAD_CHECK{'Rd-Reset-Hour-Time'};
...
sub dynamic_start{
    ...
    #With this we can specify that a user can only get so much data every 'Rd-Reset-Interval-Time' seconds
    if(exists($RAD_CHECK{'Rd-Reset-Interval-Time'})){
        $dyn_start = time-$RAD_CHECK{'Rd-Reset-Interval-Time'}; #start in the past
    }
    return $dyn_start;
}
